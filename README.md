...
| Day | 演算法/主題 | 資料集 | 學習重點與「故事劇情」 |
| :--- | :--- | :--- | :--- |
| **07** | 決策樹 (Decision Tree)| **信用貸款違約** | **模型解釋**：畫出一張銀行經理看得懂的圖。 |
| **08** | 隨機森林 (Random Forest)| **信用貸款違約** | **集成投票**：一棵樹容易誤判，種一片森林來投票 (Bagging)，大幅提升穩健度。 |
| **09** | XGBoost / LightGBM | **信用貸款違約** | **接力修正**：不再只是投票，而是針對上一棵樹「做錯的題目」加強訓練 (Boosting)。業界競賽王者。 |
| **10** | **可解釋 AI (SHAP)** | **信用貸款違約** | **解開黑箱**：XGBoost 雖然準但看不懂？引入 **SHAP Values**，視覺化告訴老闆「為什麼」模型判定這位員工會走。 |
| **11** | 主成分分析 (PCA) | **信用貸款違約** | **降維視覺化**：資料有 20 個維度畫不出來？壓成 2D 散佈圖，看離職群體是否聚在一起。 |
| **12** | K-Means 分群 | **信用貸款違約** | **自動分群**：拿掉標籤。能不能自動把員工分成「高薪懶散族」或「低薪過勞族」？ |
