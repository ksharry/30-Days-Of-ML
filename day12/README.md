...
#### 2.4 交叉驗證 (Cross Validation) - 考試不能只考一次
...
#### 2.5 分層留一法 (Stratified LOOCV) - 小樣本救星
*   **問題**：如果資料超級少 (例如只有 50 筆)，切成 10 份每份只剩 5 筆，訓練效果會很差。
*   **解法**：**Leave-One-Out Cross Validation (LOOCV)**。
    *   **極致的切法**：每次只留 **1 筆** 當測試，剩下 49 筆訓練。
    *   **重複 N 次**：總共做 50 次實驗，最後算平均。
*   **Stratified (分層)**：
    *   如果這 50 筆資料裡，只有 5 筆是「有病」(類別不平衡)。
    *   **Stratified** 會確保每次切分時，訓練集和測試集的類別比例都跟原始資料一樣 (雖然 LOOCV 每次只留 1 筆，但在 K-Fold 中 Stratified 非常重要)。
    *   **結論**：資料極少時，**LOOCV** 是最佳選擇；若類別不平衡，記得用 **Stratified** 版本。
