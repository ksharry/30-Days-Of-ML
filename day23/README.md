...
神經元處理訊號的過程是 **「先加總，再激活」**，這是一個連續的動作：

**流程圖**： `輸入 (x) --> [加權總和 (z)] --> [激活函數 (a)] --> 輸出`

1.  **第一步：加權總和 (Weighted Sum)**
    *   **公式**：`z = (w1*x1 + w2*x2) + b`
    *   **意義**：收集所有輸入訊號的強度。這就像是計算「總分」。
    *   *注意：這時候的 `z` 還只是線性疊加，沒有解決 XOR 的能力。*

2.  **第二步：激活函數 (Activation Function)**
    *   **公式**：`a = ReLU(z)` 或 `a = Sigmoid(z)`
    *   **意義**：把剛剛算出來的「總分 `z`」拿來判斷。
    *   **關聯**：**`z` 是激活函數的輸入**。例如 ReLU 函數的規則是：**「如果 `z` 是負的，就輸出 0；如果是正的，就直接輸出 `z`」**。這一步賦予了神經元「轉彎」的能力。
