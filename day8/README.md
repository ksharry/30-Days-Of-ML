...
### 核心公式與參數 (Formula & Parameters)

雖然隨機森林沒有像線性回歸那樣簡單的數學公式，但我們可以從 **「運作流程」** 和 **「重要參數」** 來理解它：

**1. 運作公式 (The Math)**
隨機森林的預測結果 $H(x)$ 是 $N$ 棵樹預測結果的整合：
*   **分類問題**：採用 **多數決 (Majority Voting)**。
    $$H(x) = \text{argmax}_c \sum_{i=1}^N I(h_i(x) = c)$$
    (100 棵樹投票，票數最多的類別勝出)
*   **回歸問題**：採用 **平均值 (Averaging)**。
    $$H(x) = \frac{1}{N} \sum_{i=1}^N h_i(x)$$
    (把大家的預測結果加起來取平均)

**2. 關鍵參數 (Key Hyperparameters)**
這就像是調整火箭引擎的旋鈕：

| 參數 | 意義 | 調整建議 |
| :--- | :--- | :--- |
| **`n_estimators`** | **樹的數量** | 越多越穩，但計算越久。建議從 100 開始。 |
| **`max_depth`** | **樹的深度** | 限制視野，避免鑽牛角尖 (Overfitting)。 |
| **`max_features`** | **特徵數量** | 每棵樹只能看部分特徵，增加多樣性。 |
| **`min_samples_split`** | **分裂門檻** | 節點樣本太少就不再切分，讓模型更保守。 |
...
## 3. 實戰：信用違約預測
### Python 程式碼實作
完整程式連結：[RandomForest.py](https://github.com/ksharry/30-Days-Of-ML/blob/main/day8/RandomForest.py)

### 整體成績單 (Performance Metrics)
首先，我們來看模型的核心指標。這些數字告訴我們，模型並沒有「死記硬背」（過擬合），因為訓練分數與測試分數相當接近。

* 🔹 **訓練集準確率 (Training Acc): 1.00** 
* 🔹 **測試集準確率 (Test Acc): 0.76** 
...
* **在 Day 08 的實戰中**：
    ...
    * 這樣的模型既保留了決策樹的判斷力，又消除了它的暴衝性格，最終在測試資料上達到 **穩定運行 (High Accuracy & High Recall)**。
