...
*   **分數 (Loss)**：Loss D: 0.62 | Loss G: 0.65 (雙方勢均力敵，都在瞎猜)
...
*   **分數 (Loss)**：Loss D: 0.43 | Loss G: 2.04 (警察變強了，生成器被電爆)
*   **標準答案 (Real)**：清晰的手寫數字。
...
#### **後期 (Epoch 50)**
![Epoch 50](pic/comparison_epoch_50.png)
*   **分數 (Loss)**：Loss D: 0.33 | Loss G: 2.02 (警察太強，生成器跟不上)
*   **標準答案 (Real)**：清晰的手寫數字。
*   **目前猜測 (Fake)**：雖然有形狀，但還是糊糊的 (與真實差異大)。
*   **警察反應**：「勉強給過...但還是看得出破綻。」
*   **原因**： $G$ 已經學會了 MNIST 的數據分佈， $D$ 逼迫它畫出細節。

> **Q: 為什麼 Epoch 50 還是跟真圖差這麼多？(關鍵)**
> 這就是 GAN 最難的地方 —— **訓練不平衡 (Imbalanced Training)**。
> *   **數據解讀**：你看 Epoch 50 的分數，`Loss D (0.33)` 很低，代表**警察太強了**，它一眼就看出假圖。而 `Loss G (2.02)` 很高，代表**生成器一直在被罵**。
> *   **後果**：當警察太嚴格時，生成器會感到「挫折」，它發現不管怎麼畫都被抓，最後只能畫出這種「保守、模糊」的圖案來勉強交差。
> *   **解決方案**：這只是最簡單的 GAN (用全連接層)。如果改用 **DCGAN (用 CNN)** 或 **Diffusion Model (Day 35)**，畫質就會跟真的一樣了！
...
Day 34 我們將介紹 **VAE (Variational Autoencoder)**。
它和 GAN 一樣能生成圖片，但原理完全不同。
GAN 靠「騙」，VAE 靠「壓縮」。

## 6. IPAS 考題補充 (GenAI 相關)
以下彙整了 114 年 IPAS 中級能力鑑定中與生成式 AI 相關的考題，供複習參考：

| 題目 (關鍵字) | 答案 | 解析 |
| :--- | :--- | :--- |
| **20. GAN Mode Collapse** | (B) 採用 Wasserstein 距離 (WGAN) | 模式崩塌 (Mode Collapse) 是 GAN 只生成單一種類圖片的現象，WGAN 可緩解此問題。 |
| **21. 多模態缺失處理** | (B) 訓練具備模態缺失感知能力的模型 | 處理如「有圖無文」或「有文無圖」的情況。 |
| **35. CLIP 特性** | (A) 圖文對比學習，零樣本分類 | CLIP 透過對比學習 (Contrastive Learning) 將圖片與文字對齊。 |
| **40. 生成模型差異** | (A) VAE 模糊，GAN 不穩，Diffusion 穩定 | VAE 生成較模糊但連續；GAN 清晰但不穩；Diffusion 清晰且穩定但慢。 |
| **44. 預測與生成兼顧** | (C) VAE 或 GAN | 這些模型既能學到資料分佈 (生成)，也能學到特徵表示 (預測)。 |
| **(第三科) 50. 多模態細節錯誤** | (C) CLIP 編碼器在語意空間未充分對齊 | CLIP 有時會忽略細節 (如數量、方位)，因為它學的是整體語意對齊。 |
