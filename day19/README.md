...
#### 2.1 什麼是 Boosting？ (30 個特徵怎麼考？)
不同於 Random Forest (Bagging) 是大家「同時」投票，Boosting 是「接力」進行的。
想像這 30 個特徵 (腫瘤大小、硬度...) 是 30 種不同的檢驗項目：

1.  **第一棒 (Model 1)**：
    *   它從 30 個特徵中，選出**最能區分良性/惡性**的那 **1 個**特徵 (例如：`worst area` 腫瘤面積)。
    *   它說：「只要面積大於 X 就是惡性！」
    *   **結果**：大部分對了，但有些「面積小但很硬」的惡性腫瘤被漏掉了 (預測錯誤)。
2.  **權重調整 (劃重點)**：
    *   那些被漏掉的「面積小但很硬」的案例，會被**加重權重** (變成考試必考題)。
3.  **第二棒 (Model 2)**：
    *   它看到那些「必考題」很重要，於是選了**另一個特徵** (例如：`worst texture` 硬度) 來區分它們。
    *   它說：「只要硬度大於 Y 就是惡性！」(這樣就抓到了第一棒漏掉的那些)。
4.  **接力下去**：
    *   每一棒都針對前一棒沒做好的地方，選一個最適合的特徵來切一刀。
    *   最後把這 50 個「單科專家」的意見加權總合起來，就變成一個全能的名醫了！
...
#### 2.4 核心公式 (Math behind the Magic)
如果你對數學感興趣，這裡有兩個關鍵公式：

**1. 這一棒講話可以多大聲？ (模型權重 $\alpha_t$)**
