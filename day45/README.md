...
## 5. IPAS 考題詳解 (VGG16 架構篇)
這是一題非常經典的 VGG16 架構題 (Q44)，完全考驗你對 `torchsummary` 的理解。

| 題目 | 答案 | 解析 |
| :--- | :--- | :--- |
| **44. VGG16 架構細節** | **(D)** | **(A) 錯誤**：AdaptiveAvgPool2d 會強制將輸出轉為 **7x7** (而非 4x4)，所以輸入維度是 `512 * 7 * 7 = 25,088`。<br>**(B) 錯誤**：Param # 當然包含 **偏差 (Bias)**。<br>**(C) 錯誤**：625MB 僅是模型參數與單次前向傳播的估算。訓練時還需要儲存 **梯度 (Gradients)**、**優化器狀態 (Optimizer States)**，通常需要 3~4 倍以上的記憶體，1GB 絕對不夠。<br>**(D) 正確**：VGG16 名稱由來就是 **13層卷積 + 3層全連接 = 16層**。總參數 1.38億也吻合。 |
