...
## 5. 戰略總結:模型訓練的火箭發射之旅
最後，讓我們引用 AI 大師 **吳恩達 (Andrew Ng)** 的經典圖表，來重新審視我們學到的XGBoost模型：
![rocket](https://github.com/ksharry/30-Days-Of-ML/blob/main/day2/pic/2-6.jpg?raw=true)

XGBoost模型就像威力強大的火箭引擎，參數設得好能飛天，設不好會直接爆炸。
...
* **設定**：適中的樹深 (3-6)和適中的學習率 (0.01-0.1)。
* **第一關 & 第二關**：
    * 訓練集與測試集表現都很好 (**Test Acc ≈ 77.33%**)。
...
**Next Day 預告：**
明天 Day 10，我們將探討無監督學習中的經典算法 —— **主成分分析 (PCA)**。
面對高維度的資料（例如幾百個特徵），模型往往會陷入「維度詛咒」，計算變慢且容易過擬合。
PCA 能像魔術師一樣，在保留資料主要特徵的同時，將繁雜的數據壓縮到低維度。這不僅能加速訓練，還能讓我們將高維數據視覺化，直接「看見」資料的結構！