...
## 2. 原理：解構最寬馬路 (The Mathematics of Margins)
...
$$\mathbf{w} \cdot \mathbf{x} + b = 1 \quad (\text{正向邊緣})$$


$$\mathbf{w} \cdot \mathbf{x} + b = -1 \quad (\text{負向邊緣})$$
...
### 進階實戰：參數的暴力破解 (Grid Search)
完整程式連結：[SVM_Tuning.py](https://github.com/ksharry/30-Days-Of-ML/blob/main/day6/SVM_Tuning.py)

在原本的實驗中，我們使用預設參數 (`C=1.0`) 得到了不錯的結果。但身為資料科學家，我們不能只滿足於「預設值」。我們使用了 **Grid Search (網格搜索)** 技術，讓電腦自動測試了 100 種不同的參數組合，最終找到了這組參數：

* **最佳組合**：`{'C': 10, 'gamma': 0.1}`


### 參數深度解析
![Tuning1](https://github.com/ksharry/30-Days-Of-ML/blob/main/day6/pic/6-4.jpg?raw=true)
![Tuning2](https://github.com/ksharry/30-Days-Of-ML/blob/main/day6/pic/6-5.jpg?raw=true)

為什麼電腦會選這組參數？這背後有它的物理意義：

1.  **`C = 10` (嚴格的教官)**：
    * 預設是 1，電腦選擇了 10。這代表模型決定對「錯誤」採取**較嚴厲的懲罰**。
    * 在鐵達尼號的生死邊界上，它不希望有太多模糊地帶，因此它會試著把決策邊界收得更緊一點，去包住那些原本容易被分錯的乘客（例如：坐三等艙但票價偏高的案例）。

2.  **`gamma = 0.1` (適中的視力)**：
    * 這個值非常接近公式推算的自然值 (1/特徵數)。
    * 這代表模型認為：判斷一個乘客的生死，不需要看太遠（太遠會失焦），也不需要只看貼在身邊的人（太近會鑽牛角尖）。**0.1 是一個剛好能看清局部特徵，又能保持大局觀的完美平衡。**


## 5. 戰略總結:模型訓練的火箭發射之旅

最後，讓我們引用 AI 大師 **吳恩達 (Andrew Ng)** 的經典圖表，來重新審視我們學到的模型，在 Day 06 中，我們不再只是調整燃料（數據），而是直接對火箭的 **引擎構造 (Kernel)** 與 **導航系統 (Parameters)** 進行了精密改裝。

![Model Training and Tuning Process](https://github.com/ksharry/30-Days-Of-ML/blob/main/day2/pic/2-6.jpg?raw=true)

### 5.1 流程一：推力不足，無法升空 (Underfitting 迴圈)

* **設定**：假設我們一開始只用了 **線性核函數 (Linear Kernel)**，或者 $C$ 值設得太小 (例如 0.01)。
* **第一關：訓練集表現好嗎？**
    * **否 (No)**。
    * **診斷**：**欠擬合 (Underfitting)**。
        * **原因**：這就像是用螺旋槳引擎去推火箭。線性邊界太簡單，切不開複雜的鐵達尼號數據（那些混在一起的紅豆綠豆）。
* **行動 (Action)**：箭頭指向 **「調整參數：增加複雜度」**。
    * **升級引擎**：從 Linear 換成 **RBF 核函數**（開啟高維空間魔法）。
    * **加強馬力**：**增大 $C$ 值**（減少對錯誤的容忍，強迫模型學得更精細）。

### 5.2 流程二：動力太強，失控亂飛 (Overfitting 迴圈)

* **設定**：假設我們把 $C$ 設為 1000，且 $\gamma$ 設為 100。
* **第一關：訓練集表現好嗎？**
    * **是 (Yes)**。模型會在每一個訓練數據點周圍畫出獨立的小圈圈，拿到 100% 分數。
* **第二關：測試集表現好嗎？**
    * **否 (No)**。
    * **診斷**：**過擬合 (Overfitting)**。
        * **原因**：火箭動力太強且過度敏感。因為 $\gamma$ 太大（高度近視），模型對每一個雜訊都做出反應，導致邊界破碎支離。
* **行動 (Action)**：箭頭指向 **「調整參數：限制複雜度」**。
    * **放鬆管制**：**減小 $C$ 值**（允許一些誤判，換取更平滑的邊界）。
    * **擴大視野**：**減小 $\gamma$ 值**（讓模型看大局，不要鑽牛角尖）。

### 5.3 流程三：完美入軌 (The Sweet Spot)

* **設定**：Grid Search 幫我們找到了 **$C=10, \gamma=0.1$**。
* **第一關 & 第二關**：
    * 訓練集與測試集表現都很好 (**Acc ≈ 82.12%**)。
* **結果**：**完成！**
    * 這組參數就像是找到了最佳的推力與最穩定的導航系統。我們成功在「分得開」與「分得穩」之間取得了完美的平衡。

---

## 6. 總結與比較 (Conclusion)

經過了 Grid Search 的優化，讓我們看看目前的最終排行榜：
...
雖然準確率只提升了約 0.5%~1%，但在機器學習競賽中，這往往是決定名次的關鍵差距。更重要的是，SVM 透過 **C (正則化)** 與 **Gamma (核函數)** 的配合，展現了比前兩者更強大的穩健性。
...
### Next Step:
到目前為止，我們的模型（係數、鄰居、高維空間）對人類來說都越來越難以理解了，變成了「黑盒子」。


---

## 9. 補充資料：重點觀念懶人包 (Cheat Sheet)

為了方便大家了解，這裡整理了 SVM 最核心的兩個概念：**核函數的選擇** 與 **Gamma 參數的計算**。

### 9.1 四大核函數 (Kernels) 比較表
當我們在設定 `kernel='rbf'` 時，其實還有其他選擇。這張表整理了 sklearn 中常見的核函數及其幾何意義：

* **Linear (線性核)**：一把直尺。適合特徵很多或原本就是線性的資料。
* **Poly (多項式核)**：彎曲的紙。適合有互動特徵 ($x_1 \times x_2$) 的資料。
* **RBF (徑向基核)**：**預設且最強**。像是在紙上吹氣泡，能處理絕大多數非線性問題。
* **Sigmoid 核**：S 型曲面。模擬神經網路，現在較少使用。

### 9.2 Gamma='scale' 到底是多大？
在程式碼中我們設定 `gamma='scale'`，這到底代表數值是多少？

* **自動標準化**：這個公式 $\frac{1}{n\_features \times X.var()}$ 非常聰明。
* 它會根據你的 **特徵數量 (n_features)** 和 **資料分佈範圍 (Variance)** 自動調整 Gamma 的大小。
* 這就是為什麼在我們的鐵達尼號案例中，Grid Search 算出來的最佳 Gamma (0.1) 會跟預設的 `scale` (約 0.125) 這麼接近的原因！