...
---
## 3. 理論基礎：解構貝氏定理 (The Mathematics)

貝氏分類器的核心當然是 **貝氏定理 (Bayes' Theorem)**。讓我們把這個數學公式拆解開來，看看它在垃圾郵件過濾中代表什麼意義。
...
| 數學術語 | 代表意義 | 在本案中的解釋 |
| :--- | :--- | :--- |
| **Posterior (後驗機率)** | $P(\text{Spam} \| \text{Words})$ | **我們的目標**。看到這封信的文字內容後，它是垃圾信的機率是多少？ |
| **Likelihood (似然性)** | $P(\text{Words} \| \text{Spam})$ | **模型的學習核心**。如果已知是垃圾信，出現這些特定關鍵字（如 "Free", "Win"）的機率有多大？這就是模型從訓練集裡統計出來的數字。 |
| **Prior (先驗機率)** | $P(\text{Spam})$ | **基礎機率**。在沒看內容之前，隨便抓一封信是垃圾信的機率。例如 UCI 資料集中，垃圾信約佔 13.4%。 |
| **Evidence (證據)** | $P(\text{Words})$ | **標準化常數**。這封信的文字組合在所有信件中出現的機率。因為在比較 $P(\text{Spam}\|\dots)$ 和 $P(\text{Ham}\|\dots)$ 時分母都一樣，通常我們會**忽略不計**。 |

### 3.3 關鍵參數：平滑係數 (Alpha)

在計算 **Likelihood (似然性)** 時，我們會遇到一個致命問題：**「零機率陷阱」**。
如果有一個詞 "Bitcoin" 在訓練資料的垃圾信裡從沒出現過，那麼 $P(\text{Bitcoin}|\text{Spam}) = 0$。根據公式，整個機率乘積都會變成 0，導致判斷失效。

為了解決這個問題，我們引入了 **拉普拉斯平滑 (Laplace Smoothing)**，也就是程式碼中的 `alpha` 參數：

$$P(w_i|C) = \frac{N_{wi} + \alpha}{N_{total} + \alpha \times V}$$

* **$N_{wi}$**：該單字在該類別出現的次數。
* **$N_{total}$**：該類別所有單字的總數。
* **$V$**：整個詞彙表的大小 (Vocabulary Size)。
* **$\alpha$ (Alpha)**：
    * **`alpha=1.0` (預設)**：給每個單字都預設「出現過 1 次」。保證機率永遠不會是 0。
    * **`alpha` 越大**：模型越平滑、越保守 (Underfitting 傾向)。
    * **`alpha` 越小**：模型越敏感 (Overfitting 傾向)。
...
### 5.1 流程一：詞彙貧乏，無法判斷 (Underfitting 迴圈)
...
### 5.2 流程二：鑽牛角尖，過度敏感 (Overfitting 迴圈)
