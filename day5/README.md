...
KNN 的運作原理非常直觀：**「計算距離，找出最近的 K 個鄰居，然後投票。」**

#### 1. 距離公式 (Distance Metric)
電腦怎麼知道誰跟誰比較「近」？最常用的方法是 **歐幾里得距離 (Euclidean Distance)**，也就是國中學過的「畢氏定理」推廣版。

假設有兩個乘客 A 和 B，他們的特徵分別是 $(x_1, y_1)$ 和 $(x_2, y_2)$：
$$ d(A, B) = \sqrt{(x_1 - x_2)^2 + (y_1 - y_2)^2} $$

如果有更多特徵 (例如年齡、票價、艙等...)，公式就推廣為：
$$ d(A, B) = \sqrt{\sum_{i=1}^n (A_i - B_i)^2} $$

#### 2. 關鍵參數 (Hyperparameters)
在使用 KNN 時，有兩個最重要的參數決定了模型的成敗：

| 參數名稱 | 意義 | 調整影響 |
| :--- | :--- | :--- |
| **`n_neighbors` (K)** | **鄰居數量** | **K 太小 (如 1)**：容易受雜訊影響，過擬合 (Overfitting)。<br>**K 太大 (如 100)**：過度平滑，忽略局部特徵，欠擬合 (Underfitting)。 |
| **`p` (Distance)** | **距離算法** | **p=2 (預設)**：歐幾里得距離 (直線距離)。<br>**p=1**：曼哈頓距離 (走格子的距離)。 |
...
5.3.  **決策點 2：測試/驗證集表現好嗎？ (Test/Validation Set Performance?)**
