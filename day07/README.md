...
### 6.4 時間複雜度 (Time Complexity) 與 Big O
面試常問：「KNN 的時間複雜度是多少？」答案通常是 **$O(n^2)$** (如果算所有兩兩距離)。
這裡補充常見的 Big O 意義：

*   **(A) $O(n)$ 線性關係**
    *   **意義**：資料變 2 倍，時間就變 2 倍。
    *   **場景**：找出「年齡最大」的客戶（只需把每個人看過一遍）。
*   **(C) $O(1)$ 常數時間**
    *   **意義**：不管資料有多少，時間都一樣（瞬間完成）。
    *   **場景**：存取陣列中的第 5 個元素。
*   **(D) $O(\log n)$ 對數關係**
    *   **意義**：資料變很多，時間只增加一點點（非常高效）。
    *   **場景**：二分搜尋法（Binary Search），在已排序的資料中找特定人。
*   **(B) $O(n^2)$ 平方關係**
    *   **關鍵字**：「兩兩比對」、「所有組合」、「雙層迴圈」、「矩陣計算」。
    *   **KNN 痛點**：因為要拿「測試點」跟「所有訓練點」算距離，所以當資料量大時，KNN 會變得非常慢。
