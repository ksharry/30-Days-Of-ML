...
### 5.1 目前進度統計
我們已經將大部分的考題分散到各個對應的章節中，以便在學習該主題時能直接對照練習。以下是目前的補充狀況：

*   **第一科 (人工智慧技術應用與規劃)**：共 50 題，已補充 **34** 題。
    *   主要集中在：NLP (Day 31), CV (Day 25/32), GenAI (Day 33), MLOps (Day 42), Security (Day 43)。
*   **第三科 (機器學習技術與應用)**：共 50 題，已補充 **11** 題。
    *   主要集中在：Evaluation (Day 12), CNN (Day 25), Bias (Day 43)。

### 5.2 待加強領域 (Gap Analysis)
從統計數據可以看出，我們在 **第三科 (機器學習技術與應用)** 的補充比例較低。這部分的題目偏向 **基礎理論與數學細節**，例如：
*   **基礎演算法細節**：KNN 複雜度、決策樹資訊增益、SVM 核函數。
*   **數學原理**：矩陣運算、條件機率、微積分 (梯度下降)。
*   **特徵工程細節**：多重共線性、特徵交互作用。

### 5.3 最後衝刺建議
針對尚未補充的題目 (標示為 Day 01-11, Day 15-24 的部分)，建議採取以下策略：
1.  **回歸基礎**：重新複習 Day 01-10 的 `README.md`，特別是數學公式的部分。
2.  **實作驗證**：對於不確定的參數影響 (如 Learning Rate, Batch Size)，直接修改對應天數的 Python 腳本跑跑看。
3.  **學習指引 (Study Guide)**：官方提供學習文件-AI應用規劃師(中級)-學習指引