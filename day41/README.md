...
| **10. Precision & Recall 綜合** | (D) F1 分數 | Day 12 (Evaluation) |
| **11. DBSCAN 超參數** | (C) 鄰域半徑 (Epsilon) 與最小點數 (MinPts) | Day 15 (DBSCAN) |
| **12. 多重共線性處理** | (B) 使用 PCA 轉換為獨立主成分 | Day 16 (PCA) |
| **13. Kubernetes 功能** | (B) 管理模型服務的部署與擴展 | Day 42 (MLOps) |
| **14. 防止過擬合 (超參數)** | (A) 採用交叉驗證 (Cross-Validation) | Day 12 (Evaluation) |
| **15. Model Registry 用途** | (C) 集中管理模型版本與部署狀態 | Day 42 (MLOps) |
...
| **22. 資料漂移偵測** | (D) 計算 KL 散度 (KL Divergence) | Day 42 (MLOps) |
| **23. 漸進式部署** | (A) 從單一專科開始，逐步擴展 | Day 42 (MLOps) |
| **24. 對抗性攻擊防禦 (非)** | (D) 強化防火牆 (這是資安層面) | Day 43 (Security) |
| **25. 生成內容侵權預防** | (B) 建立訓練資料篩選與授權驗證 | Day 43 (Security) |
| **26. 共線性模型選擇** | (D) 含 L1 正則化的 LASSO 迴歸 | Day 04 (Regularization) |
| **27. JSON 日誌特徵提取** | (C) 展開巢狀欄位並基於時間窗口聚合 | Day 42 (MLOps) |
| **28. 混合特徵處理** | (C) 連續特徵標準化，類別特徵目標編碼 | Day 11 (Feature Eng) |
| **29. CI 核心實踐** | (B) 提交後自動觸發建置與測試 | Day 42 (MLOps) |
| **30. 不可否認性** | (A) 記錄雜湊值並簽署數位簽章 | Day 43 (Security) |
| **31. 高流量架構** | (B) 容器化部署並水平擴展 (Auto Scaling) | Day 42 (MLOps) |
| **32. 效能衰退預警** | (D) 輸入特徵分佈的 PSI 指數 | Day 42 (MLOps) |
...
| **39. ARIMA 殘差自相關** | (C) 模型配適不足，需調整 p, q 參數 | Day 44 (Time Series) |
| **40. 生成模型差異** | (A) VAE 模糊，GAN 不穩，Diffusion 穩定 | Day 33-35 (GenAI) |
| **41. 交叉驗證資料洩漏** | (A) 測試摺參與參數選擇 | Day 12 (Evaluation) |
| **42. 新樣本分佈偏移** | (D) 使用 VAE 監控潛在空間分佈 | Day 42 (MLOps) |
| **43. 模型比較實驗** | (B) 低資源情境下比較 | Day 12 (Evaluation) |
| **44. 預測與生成兼顧** | (C) VAE 或 GAN | Day 33 (GenAI) |
| **45. PCA + SVM** | (D) 降維可降低訓練時間並減少過擬合 | Day 10 (SVM) |
| **46. MLOps 漂移偵測** | (A) 建立即時 Data/Concept Drift 監測 | Day 42 (MLOps) |
| **47. 多任務學習衝突** | (C) 損失函數未平衡 | Day 24 (Deep Learning) |
| **48. DBSCAN 加速** | (B) 使用 KD-Tree 或 Ball Tree 索引 | Day 15 (DBSCAN) |
| **49. 跨文化情感分析 (非)** | (A) 詞嵌入正規化導致誤差 | Day 43 (Bias) |
| **50. 多模態細節錯誤** | (C) CLIP 編碼器在語意空間未充分對齊 | Day 33 (GenAI) |
...
| 題目 (關鍵字) | 答案 | 對應天數 (Day) |
| :--- | :--- | :--- |
| **1. 模型穩定性檢驗** | (B) **交叉驗證 (Cross-Validation)** | Day 12 (Evaluation) |
...
| **13. 標籤偏差原因** | (B) 標記資料帶有主觀偏見 | Day 43 (Bias) |
...
| **21. 多頭注意力優點** | (C) 從不同子空間捕捉多樣化關聯 | Day 31 (Transformer) |
...
| **28. 取樣偏差** | (C) 訓練樣本僅涵蓋高活躍顧客 | Day 43 (Bias) |
...
| **34. 小樣本驗證** | (D) 分層留一法 (Stratified LOOCV) | Day 12 (Evaluation) |
| **35. PCA 解釋變異** | (A) 90% > 80%，前兩維足夠 | Day 16 (PCA) |
| **36. 同態加密特性** | (D) 加密狀態下仍可進行運算 | Day 43 (Security) |
| **37. 安全多方計算** | (D) 同態加密 + MPC + 雜湊 + 對稱加密 | Day 43 (Security) |
...
| **43. VGG16 運算量最多** | (A) 卷積層 (Conv2d) | Day 25 (CNN) |
