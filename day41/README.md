...
| **13. Kubernetes 功能** | (B) 管理模型服務的部署與擴展 | Day 29 (Deployment) |
| **14. 防止過擬合 (超參數)** | (A) 採用交叉驗證 (Cross-Validation) | Day 12 (Evaluation) |
| **15. Model Registry 用途** | (C) 集中管理模型版本與部署狀態 | Day 29 (MLOps) |
...
| **23. 漸進式部署** | (A) 從單一專科開始，逐步擴展 | Day 29 (Deployment) |
| **24. 對抗性攻擊防禦 (非)** | (D) 強化防火牆 (這是資安層面) | Day 33 (GAN) |
| **25. 生成內容侵權預防** | (B) 建立訓練資料篩選與授權驗證 | Day 34 (Ethics) |
| **26. 共線性模型選擇** | (D) 含 L1 正則化的 LASSO 迴歸 | Day 04 (Regularization) |
| **27. JSON 日誌特徵提取** | (C) 展開巢狀欄位並基於時間窗口聚合 | Day 27 (Time Series) |
| **28. 混合特徵處理** | (C) 連續特徵標準化，類別特徵目標編碼 | Day 11 (Feature Eng) |
| **29. CI 核心實踐** | (B) 提交後自動觸發建置與測試 | Day 29 (MLOps) |
...
| **13. 標籤偏差原因** | (B) 標記資料帶有主觀偏見 | Day 34 (Bias) |
...
| **28. 取樣偏差** | (C) 訓練樣本僅涵蓋高活躍顧客 | Day 34 (Bias) |
...
| **36. 同態加密特性** | (D) 加密狀態下仍可進行運算 | Day 34 (Privacy) |
| **37. 安全多方計算** | (D) 同態加密 + MPC + 雜湊 + 對稱加密 | Day 34 (Privacy) |
