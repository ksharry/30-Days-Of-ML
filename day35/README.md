...
> **Q: U-Net 是把所有圖都背下來了嗎？雜訊組合不是無限多嗎？**
> 這是一個非常棒的問題！
> *   **不是死背 (Not Memorization)**：U-Net 學的是**「規律」 (Distribution)**，而不是像素。
>     *   **比喻**：就像學畫貓。你不是背下全世界每一隻貓的長相，而是學會「貓有尖耳朵、有鬍鬚、有尾巴」。
>     *   當 U-Net 看到一堆雜訊 (牆上的壁癌) 剛好有一點點像尖耳朵，它就會根據學到的規律，把那裡修補成耳朵。
> *   **雜訊無限多 = 創意無限大**：
>     *   正因為雜訊的組合是無限的 (點、線、面的隨機排列)，U-Net 每次看到的「起點」都不一樣。
>     *   所以它每次「腦補」出來的結果也都不同。這就是為什麼它能**無中生有**，創造出世界上不存在的圖片。
>
> **Q: 它怎麼知道要補「橘色」還是「黑色」的耳朵？它是如何記錄的？**
> 這取決於你給它的**提示 (Conditioning)**：
> 1.  **無提示 (Unconditional，像我們的 MNIST)**：
>     *   **靠機率**：它看過 60,000 張圖，發現「這種尖尖的形狀，通常是白色的 (因為 MNIST 只有黑白)」。所以它會自動補成白色。
>     *   **隨機性**：如果是彩色貓咪資料集，可能有黑貓也有橘貓。那它就會**隨機**選一種補上去 (這次補黑的，下次可能補橘的)。
> 2.  **有提示 (Conditional，像 Stable Diffusion)**：
>     *   **靠指令**：你會給它一個文字指令 (Prompt)：「一隻**橘色**的貓」。
>     *   **Cross-Attention (注意力機制)**：這個指令會變成一個向量，像「指揮官」一樣告訴 U-Net：「喂！這裡雖然也可以補黑色，但主人說要橘色，所以請你把這裡的雜訊修成橘色！」
>     *   這就是為什麼現在的 AI 可以聽懂人話來畫圖。
