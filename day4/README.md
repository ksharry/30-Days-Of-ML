...
*   **圖表**：
    ...
    *   **殘差圖比較**：比較三種模型的誤差分佈。
        *   **殘差怎麼算？** $殘差 = 真實值 - 預測值$。也就是模型「猜錯」了多少。
        *   **為什麼要在紅線 (0) 附近？** 紅線代表殘差為 0 (完全猜對)。如果藍點均勻散佈在紅線上下 (有正有負，平均為 0)，代表模型沒有偏見 (Bias)。
        *   **什麼是不正常？** 如果藍點呈現 U 型、漏斗型，代表數據裡還有某種「規律」模型沒學到 (例如非線性關係)，這時線性模型就不夠用了。
        *   **本案異常**：你有發現圖中有一條明顯的 **「斜切線」** 嗎？這是因為加州房價數據中，房價被「截斷 (Capped)」在 50 萬美金 (Target=5.0)。所有超過 50 萬的房子都被記為 50 萬，這導致模型預測高價房時出現了系統性的誤差。這也提醒我們：**數據本身的品質 (如截斷問題) 往往比模型選擇更重要！**
...
![rocket](day2/pic/2-4_Rocket.jpg)
