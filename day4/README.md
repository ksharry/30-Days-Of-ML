...
![Rocket](https://github.com/ksharry/30-Days-Of-ML/blob/main/day4/pic/4-4.jpg?raw=true)

**流程 1：起飛失敗 (欠擬合 / Underfitting)**
* **症狀**：訓練集分數就很低 (例如 60%)，連基本規律都抓不到。
* **診斷**：引擎太小，推不動。因為邏輯回歸只能畫「直線」，但真實世界的生存邊界可能是圓的。
* **維修 (調整參數)**：**進行特徵擴充 (Polynomial Features)**。
    * 我們把「年齡」變成「年齡的平方」，把「票價」變成「票價的立方」。這能讓邏輯回歸具備畫出**曲線**的能力，相當於升級了引擎。

**流程 2：空中解體 (過擬合 / Overfitting)**
* **症狀**：訓練集分數超高 (95%)，但測試集分數很低 (70%)。
