...
## 1. 資料集來源
### 資料集來源：[Synthetic Mall Data (模擬商場資料)]
...
*   **黃色大點**：質心 (Centroids)。
*   **五大族群分析 (基於模擬資料)**：
    *(註：由於使用隨機生成的模擬資料，每次跑出來的群聚位置可能略有不同，以下為典型範例)*
    1.  **低收入、低消費** (謹慎型)：收入與消費分數都偏低。
    2.  **低收入、高消費** (享樂型)：收入不高但很敢花錢。
    3.  **中等收入、中等消費** (大眾型)：人數最多的一群。
    4.  **高收入、低消費** (省錢型)：有錢但不愛花。
    5.  **高收入、高消費** (VIP)：**商場最愛的金主！**
