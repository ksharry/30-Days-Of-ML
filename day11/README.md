...
#### 2.3 互動特徵 (Interaction Features)
*   **概念**：有時候單看一個特徵看不出名堂，但把兩個特徵結合起來看，就會發現新大陸。這就是「互動特徵」。
*   **例子**：
    *   **房地產**：單看「長度」或「寬度」可能無法準確預測房價，但「長度 x 寬度 = 面積」就是一個超強的特徵。
    *   **醫療**：單看「身高」或「體重」可能無法判斷健康風險，但「體重 / 身高平方 = BMI」就是關鍵指標。
*   **方法**：
    *   **多項式特徵 (Polynomial Features)**：將特徵相乘 (x1 * x2) 或平方 (x1^2)。
    *   **手動組合**：根據領域知識 (Domain Knowledge) 進行加減乘除。
*   **IPAS 考題重點**：考題常問「如何處理特徵間的非線性關係？」或「如何增強模型對複雜模式的捕捉能力？」，答案通常就是**建立互動特徵 (Interaction Features)**。
