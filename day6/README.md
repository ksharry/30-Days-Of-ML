...
*   **指標數字**：
    ...
    *   **Precision (精確率)**: `0.8889` - 預測為「會買」當中，真的「會買」的比例。適用於「寧可漏抓，不可錯殺」 (e.g. 垃圾郵件，不想把重要信件誤判為垃圾)。
    *   **Recall (召回率)**: `0.7500` - 實際「會買」當中，被抓出來的比例。適用於「寧可錯殺，不可漏抓」 (e.g. 癌症篩檢，不想漏掉任何一個病人)。
    *   **F1-Score**: `0.8136` - Precision 和 Recall 的調和平均，綜合指標。
    *   **AUC (Area Under Curve)**: `0.9540` - ROC 曲線下的面積。0.5 代表亂猜，1.0 代表完美預測。
    *   **Confusion Matrix**: 
        ```
        [[65,  3],  <- 預測不買 (TN=65, FP=3)
         [ 8, 24]]  <- 預測會買 (FN=8, TP=24)
        ```
        (65人真的沒買且猜對，24人真的買了且猜對；只有 3+8=11 人猜錯)
*   **圖表**：
