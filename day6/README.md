...
## 6. 進階實戰：參數的暴力破解 (Grid Search)
完整程式連結：[SVM_Tuning.py](https://github.com/ksharry/30-Days-Of-ML/blob/main/day6/SVM_Tuning.py)

在原本的實驗中，我們使用預設參數 (`C=1.0`) 得到了不錯的結果。但身為資料科學家，我們不能只滿足於「預設值」。我們使用了 **Grid Search (網格搜索)** 技術，讓電腦自動測試了 100 種不同的參數組合，最終找到了這組「神之參數」：
...
## 7. 戰略總結：模型訓練的火箭發射之旅
我們再次請出這張「火箭發射 SOP」圖。在 Day 06 中，我們不再只是調整燃料（數據），而是直接對火箭的 **引擎構造 (Kernel)** 與 **導航系統 (Parameters)** 進行了精密改裝。

![Model Training and Tuning Process](https://github.com/ksharry/30-Days-Of-ML/blob/main/day2/pic/2-6.jpg?raw=true)

### 7.1 流程一：推力不足，無法升空 (Underfitting 迴圈)
...
### 7.2 流程二：動力太強，失控亂飛 (Overfitting 迴圈)
