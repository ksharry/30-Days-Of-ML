...
## 2. 原理
### 核心概念：為什麼 MLP 不能預測股票？
*   **Day23-MLP (健忘)**：它把每一天的價格都當作獨立事件。它不知道「昨天漲了」，也不知道「前天跌了」。
*   **Day27-RNN (有記憶)**：它有一個 **「隱藏狀態 (Hidden State)」**，就像是一個 **「筆記本」**。
...
*   **核心公式**：`H_t = tanh(W * X_t + U * H_{t-1} + b)`
    *   **$W$ (藍色箭頭)**：負責處理 **今天的輸入** ($X_t$)。
    *   **$U$ (綠色箭頭)**：負責傳遞 **昨天的記憶** ($H_{t-1}$)。這就是 Loop！
    *   **$V$ (紅色箭頭)**：負責根據現在的記憶算出 **今天的輸出** ($Y_t$)。
