...
> **💡 小知識：什麼是 MCTS (蒙地卡羅樹搜尋)？**
> *   **問題**：圍棋步數太多，不可能把所有可能性都算完 (暴力搜尋會算到宇宙毀滅)。
> *   **解法**：**隨機試玩 (Simulation)**。
>     *   想像你在走迷宮，你不知道哪條路是對的。
>     *   MCTS 說：「我派 1000 隻螞蟻隨機亂跑，看哪條路最後跑出去的螞蟻最多，我就走那條。」
>     *   AlphaGo 就是用這個方法，快速判斷目前的棋局「勝率」有多大，而不需要把整盤棋算完。
...
*   **解法**：把玩過的經驗 $(S, A, R, S')$ 全部丟進一個 **「回憶庫」 (Replay Buffer)**。
    ...
    > *   **S (State)**：我看到了什麼？(例如：車子在左邊)
    > *   **A (Action)**：我做了什麼？(例如：往右推)
    > *   **R (Reward)**：結果好不好？(例如：桿子沒倒，+1 分)
    > *   **S' (Next State)**：做完動作後，變成怎樣？(例如：車子跑到中間了)
    > *   **意義**：我們把這些「日記」存起來，晚上睡覺時 (訓練時) 再隨機拿出來複習，這樣才不會只記得剛發生的事 (學壞)。
