...
當我們要合併「兩個群」時，首先要定義兩點之間的距離 (通常用歐式距離)：
...
*   **X 軸**：每個樣本 (動物)。
*   **Y 軸**：距離 (Distance)。
*   **怎麼看**：Y 軸越高的地方合併，代表這兩群差異越大。

#### 2.4 什麼時候停止？ (How to determine optimal clusters?)
他怎麼知道聚合到第幾次為最佳？其實演算法會一直聚合到剩下一大群為止。**決定「最佳群數」的權力在我們手上 (切一刀)**。

*   **視覺化切法**：請看下圖 (Dendrogram) 中的 **紅色虛線**。
    *   這條紅線就是我們的「刀」。
    *   當我們在 Y=10 的高度切下去時，紅線**穿過了幾條垂直線** (Vertical Lines)，就代表我們分出了幾群。
    *   如果把紅線往上移 (距離變大)，穿過的線變少，群數變少 (分得粗)。
    *   如果把紅線往下移 (距離變小)，穿過的線變多，群數變多 (分得細)。
*   **最佳切點**：通常會選在「最長的垂直線段」中間切一刀，因為這代表這幾群在很長一段距離內都很穩定，沒有發生合併。
